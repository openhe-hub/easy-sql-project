...
import org.easysql.info.sql.AnalyseMultiSelectPackage;
import org.easysql.info.sql.AnalyseSelectPackage;
import org.easysql.info.sql.ColumnCursor;
...
public class XmlHelper<T> {
    ...
    private int fillColumn(int startFill, int endFill, int i) {
        ...
        if (field.equals(session.getClassInfo().getIdInfo().getFieldName())) {//主键
            ...
            columnName = session.getClassInfo().getFieldInfo().get(field).getColumnName();
        }
        if (columnName != null) {
            ...
            if (!fieldName.equals("*")) {
                if (fieldName.substring(0, 2).equals("@(") &&
                    ...
                    fieldName = fieldName.equals(session.getClassInfo().getIdInfo().getColumnName()) ?
                            ...
                            session.getClassInfo().getFieldInfo().get(fieldName).getColumnName();
                }
            }
    ...
    private LinkedHashMap<Session<?>, ColumnCursor> recordSelectField(LinkedHashMap<String, Session<?>> sessions, String[] fields) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (!fieldName.equals("*")) {
                ...
                LinkedHashMap<String, FieldInfo> allField = currSession.getClassInfo().getFieldInfo();
